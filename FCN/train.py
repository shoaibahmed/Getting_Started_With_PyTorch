...
def train(batchSize, epochs, displayStep, sequenceLength, bidirectional, hiddenStateDims, numLayers, rootDir, outputDir):
	...
	# Create model
	numberOfFiles = len(dataset)
	numberOfClasses = dataset.getNumClasses()
	print ("Number of files: %d | Number of classes: %d" % (numberOfFiles, numberOfClasses))
	model = models.densenet201_finetune(numberOfClasses)
...
if __name__ == "__main__":
	...
	parser.add_option("-b", "--batchSize", action="store", type="int", dest="batchSize", default=1, help="Batch Size")
	...
	assert (options.batchSize == 1, "Error: FCN only implemented for batch size of 1")
	print (options)
